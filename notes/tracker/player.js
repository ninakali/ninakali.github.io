...
function gen_pattern(with_vol, with_controls) {
  ...
  <tr>\
  	<th>x</th>\
    <th colspan=2>Ch 1</th>\
    <th colspan=2>Ch 2</th>\
    <th colspan=2>Ch 3</th>\
    <th colspan=2>Ch 4</th>\
  </tr>'
...
function highlight(l, ch, mouse) {
	...
	if (!mouse) {
		document.getElementById(`pat${l}_${ch}`).scrollIntoView({behavior: "smooth", block: "center"});
	}
}

function unhighlight(l, ch) {
	if (l % 2) {c = "#ddd"} else {c = "#c2c2c2"};
...
function highlight_line(l) {
    ...
    document.getElementById(`pat_l${l}`).scrollIntoView({behavior: "smooth", block: "center"});
}
...
function playTimer() {
	...
	const l = cursor[0];
	setTimeout(function(){unhighlight_line(l);}, 15000/bpm);
...
function play_or_stop() {
	...
	if (playing) {
		...
		for (var i=0; i<3; i++) synths[i].triggerRelease(now);
	...
	document.addEventListener('keydown', function(event) {
	    if (!playing) {
		    if(event.keyCode == 37) {
		    	...
		    	unhighlight(cursor[0], cursor[1]);
		        cursor[0] += 1;
		        if (cursor[0] > MAX_LINES) {cursor[0] = MAX_LINES};
		        highlight(cursor[0], cursor[1]);
		    }
		    else if(event.keyCode == 39) {
		    	unhighlight(cursor[0], cursor[1]);
		        cursor[1] += 1;
		        if (cursor[1] > 3) {cursor[1] = 3};
		        highlight(cursor[0], cursor[1]);
		    }
